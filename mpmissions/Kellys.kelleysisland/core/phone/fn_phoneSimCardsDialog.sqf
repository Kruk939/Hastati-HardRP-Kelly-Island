...
createDialog "life_phone_simcards";
...
lbClear _list;
{
	...
	_list lbAdd _number;
	_list lbSetData str(_id);
	
} foreach life_phone_cardsToSell;

life_fnc_phoneSimCardsDialog_onLBChange = {
	disableSerialization;
	...
	_cardID = parseNumber _data;
	{
		...
		if(_id == _cardID) exitWith {
			...
			_price = _saldo;
			if(_isPermament) then {_isPermament = "Tak"; _price = _price + 10000;} else {_isPermament = "Nie";};
			_str = format ["Numer: %1\nFirma: %2\nSaldo: $%3\nCena za SMS: $%4\n Cena za minute: $%5\nUbezpieczona: %6\n Cena: $%7", _number, _companyID, _smsPrice, _minutePrice, _isPermament, _price];
			_ctr ctrlSetText _str;
		};
	} foreach life_phone_cardsToSell;
};
//[] spawn life_fnc_phoneGetActiveNumber;
_list ctrlSetEventHandler ["LBSelChanged", "[] spawn life_fnc_phoneSimCardsDialog_onLBChange"];
_list lbSetCurSel 0;